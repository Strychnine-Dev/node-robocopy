...
describe('command', () => {
  ...
  it('should build all options for a single destination', () => {
    const options = buildOptions('c:/destination/path');
  ...
  it('should build all options for multiple destinations', () => {
    const options = buildOptions(['c:/destination/path1', 'c:/destination/path2']);
    const result = command(options);
  ...
  it('should not set thread count if a bool', () => {
    const options = {
      source: '',
      destination: '',
      files: [],
    ...
    const args = command(options)[0].args;

    expect(args[2]).to.be('/MT');
  });

  it('should set non unicode output log', () => {
    const options = {
      source: '',
      destination: '',
      files: [],
  ...
  it('should set appending output log', () => {
    const options = {
      source: '',
      destination: '',
      files: [],
  ...
  it('should leave paths relative when configured', () => {
    const options = {
      source: 'c:/source/path/',
      destination: 'c:/destination/path/',
      files: [],
